...
The point of running this lab here, before the ALM phase, is that the integration env is the **last place a finding is cheap to fix.** Once a permission YAML or a header value lands in pre-prod via a managed solution, fixing it means another pipeline run. Three follow-ups extend that gate into the ALM phase.
...
After Lab 14 puts a site in prod, schedule a monthly run of `/scan-site` against the live URL. New CVEs and edge-case header drift land between releases; this is how you find them before the next release does. Wire it into a cron-triggered GitHub Actions workflow that opens an issue when the report changes.
...
→ [Lab 10: Source Control](../alm/10-source-control.md)
