...
│        └─ Assets/
│
├─ src/                          3. SPA source, compiled into the solution
├─ tests/                           unit tests
├─ scripts/                         sync, drift, seeding, and verification scripts
├─ package.json                     every npm run command
├─ .githooks/pre-push               blocks a push that would ship a stale bundle
├─ .gitattributes                   pins line endings for generated solution source
│
├─ docs/                         4. Documents
│
└─ .github/                      5. Automation
   ...
   └─ dependabot.yml                proposes dependency upgrades
```

Lanes 1 and 2 arrive in [Lab 02](02-environment-strategy.md). Lane 3 comes from [Appendix B](appendix-b-scripts.md) in [Lab 05](05-inner-loop.md), and lane 5 from [Appendix A](appendix-a-workflows.md) in Labs 06 and 07. On a site with no build step, lane 3 shrinks to the environment and deployment scripts only.
...
# Normalize everything Git decides is text.
* text=auto eol=lf

# Generated solution source: always LF, never touched by autocrlf.
solutions/**/*.xml   text eol=lf
solutions/**/*.yml   text eol=lf
solutions/**/*.json  text eol=lf

# Site source downloaded by the CLI.
.powerpages-site/**  text eol=lf

# Keep shell scripts and hooks executable and LF, or they fail on the runner.
*.sh                 text eol=lf
...
If the repository already has commits, renormalize once so the change lands in a single reviewable commit rather than leaking into the next feature branch:
